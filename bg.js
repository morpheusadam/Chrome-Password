...
chrome.runtime.onStartup.addListener(async function(){
	...
	chrome.storage.local.get(['key'], (result) => {
		...
		if(result.key) {
			...
			chrome.tabs.create({ url: chrome.runtime.getURL("enter-password.html") }, (tab) => {
			...
			chrome.tabs.create({ url: chrome.runtime.getURL("set-password.html") });
		}
...
function onTabUpdated(tabId, changeInfo, tab) {
    if (tabId === lastOptionsId) {
        if (changeInfo.url && changeInfo.url !== chrome.runtime.getURL("enter-password.html")) {
            //console.log(`Closing enter-password.html tab because URL changed: ${changeInfo.url}`);
...
function closeAllTabs() {
    chrome.tabs.query({}, (tabs) => {
        closedTabs = tabs
		.filter(tab => tab.url !== chrome.runtime.getURL("enter-password.html"))
		.filter(tab => tab.url !== chrome.runtime.getURL("set-password.html"))
		.map(tab => ({
            url: tab.url,
...
function reopenClosedTabs() {
	if (closedTabs.length === 0) {
		chrome.tabs.query({ url: chrome.runtime.getURL("enter-password.html") }, (tabs) => {
			tabs.forEach((tab) => {
				chrome.tabs.remove(tab.id, () => {
					//console.log('Closed set-password.html tab with ID:', tab.id);
				});
			});
		});
